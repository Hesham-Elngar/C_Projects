...
static uint32_t input_cell_postion(uint8_t arr[][MAX_SIZE])
{
    ...
    {
    case 1:
        printf(" %s <X>,ENTER CELL POSTION(1:9):",playerOfX);
        break;
    case 2:
        printf(" %s <O>,ENTER CELL POSTION(1:9):",playerOfO);
        break;
    }
...
/*Print Board and positions of cells to help players to choose cells*/
static void before_starting()
{
    uint8_t l_counter=1;
    for(int row_counter=ROW_START_INDEX; row_counter<MAX_SIZE; row_counter++)
    {
        printf("\t\t\t\t\t\t\t");
        for(int col_counter=COL_START_INDEX; col_counter<MAX_SIZE; col_counter++)
        {
            printf("%d ",l_counter++);
            if(col_counter!=2)
            {
